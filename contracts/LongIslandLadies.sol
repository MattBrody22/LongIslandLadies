...
contract LongIslandLadies is ERC165, ERC721, ERC721TokenReceiver, ERC721Metadata, Context, Ownable {
	...
	struct Ladies{string name; uint256 Experience; uint256 Attractiveness; uint256 Loyalty;  
		uint256 Kissing; uint256 intelligents; uint256 passionate; uint256 independent;}
	...
	function buildLadies(string memory nameOf) public {
		...
		uint256 rintelligents = rNumber(996);
		uint256 rpassionate = rNumber(995);
		uint256 rindependent = rNumber(994);
		Ladies memory addLadies = Ladies(nameOf,rExperience, rAttractiveness, rLoyalty, rKissing, rintelligents, rpassionate, rindependent);
		ladies.push(addLadies);
		mint(msg.sender, Total);
		emit CreateLadies(msg.sender, rExperience, rAttractiveness, rLoyalty, rKissing, rintelligents, rpassionate, rindependent);
		Total++;
	}
